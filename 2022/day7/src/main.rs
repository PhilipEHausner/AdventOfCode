...
fn solve2(lines: &Vec<String>) -> u64 {
    let tree = create_file_tree(&lines);
    tree.smallest_to_have_less_than_400k_mem()
}
            ...
            loop {
                ...
                if &list_line[..3] == "dir" {
                    ...
                    let new_child = Node {
                        ...
                        name: file_line[1].to_string(),
                        size: file_line[0].parse::<u64>().unwrap(),
                        children: vec![],
                    };
                    tree.append_child(&current_path, new_child);
                }
...
mod tests {
    ...
    fn test_solve2() {
        let lines = read_file_as_vector("./files/test.txt").expect("Error reading file.");
        assert_eq!(solve2(&lines), 24933642);
    }
}
